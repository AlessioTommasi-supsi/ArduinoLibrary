...
String viewGraph::initCirularProgressBarGraph()
{
    ...
    var_circle_progressbar_html += "        // Validazione per evitare NaN - controllo se i dati sono validi";
    var_circle_progressbar_html += "        let percentage = 0;";
    var_circle_progressbar_html += "        if (data && data.total > 0 && data.used >= 0 && !isNaN(data.total) && !isNaN(data.used)) {";
    var_circle_progressbar_html += "          percentage = (data.used / data.total) * 100;";
    ...
    var_circle_progressbar_html += "          const displayValue = isNaN(percentage) ? '0.0' : percentage.toFixed(1);";
    var_circle_progressbar_html += "          label.innerText = displayValue + '%';";
    var_circle_progressbar_html += "        }";
    var_circle_progressbar_html += "      })";
    var_circle_progressbar_html += "      .catch(error => {";
    var_circle_progressbar_html += "        console.error('Error fetching data:', error);";
    var_circle_progressbar_html += "        // In caso di errore, mostra 0%";
    var_circle_progressbar_html += "        const label = document.getElementById(labelId);";
